...
export class Prompt {
  ...
  constructor(element, notifier) {
    ...
    this.attachWysiwygToPrompt = this.attachWysiwygToPrompt.bind(this);
...
export class RubricOption {
  ...
  getFieldValues() {
      ...
      var nameString = Fields.stringField(
  ...
  addHandler() {
      ...
      var name = ItemUtilities.createUniqueName(options, 'data-option');
...
export class RubricCriterion {
  ...
  addHandler() {
      ...
      // Create the unique name for this option.
      var name = ItemUtilities.createUniqueName(criteria, 'data-criterion');
